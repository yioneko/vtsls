import { type TSLanguageService } from "@vtsls/language-service";
import {
  ClientCapabilities,
...
import { getTsLspDefaultCapabilities } from "./capabilities.js";

async function createLanguageServer() {
  const conn = createConnection(ProposedFeatures.all);

  const { createTSLanguageService, DocumentNotOpenedError, ProviderNotFoundError } = await import(
    "@vtsls/language-service"
  );

  const onServerInitialize = async (params: InitializeParams) => {
    ...
    const folders =
      ...
      (typeof root == "string" ? [{ name: root, uri: root }] : undefined);
    ...
    conn.onInitialized(() => {
      bindServiceHandlers(service, clientCapabilities);
      void initializeService();
    });

    process.on("exit", () => service.dispose());

    const capabilities = await getTsLspDefaultCapabilities();
  ...
  function bindServiceHandlers(service: TSLanguageService, clientCapabilities: ClientCapabilities) {
  ...
  conn.onInitialize(onServerInitialize);
